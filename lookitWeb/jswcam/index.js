 ...
 var jswcam = (function () {
     ...
     Library.prototype.startRecording = function (caller) {
         if (LOOKIT.is_recording) {
             ...
             console.log(
                 'Tried to start recording, but was already recording!  Stopped instead.'
             );
         }

         if (!session.hasOwnProperty('user_id') || !session.hasOwnProperty(
                 'experiment_id')) {
         ...
         swfobject.getObjectById("flashplayer").recordToCamera(
             session.experiment_id, session.user_id + '_' +
             LOOKIT.RECORDINGSET, session.participant,
             session.participant_privacy, caller, LOOKIT.recording_count
         );
