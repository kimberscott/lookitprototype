var page = (function () {
    ...
    Library.prototype.showVerbalConsentDialog = function (callback, expt) {
        ...
        var timeoutID = 0;
        var check_cam =
            "<div id = 'top_bar'><p><h1 style='text-align:center'>Test Your Webcam and Microphone </h1></p></div><div id='cam_setup'></div><div id = 'setup_message'></div>";
        bootbox.dialog(check_cam, [{
            ...
            'callback': function () {
                if (LOOKIT.doneWithConsent) {
                    ...
                    console.log(session);
                    LOOKIT.consent_recording_completed =
                        true;
                    LOOKIT.doneWithConsent = false;
                    recording = 0;
                    $("#widget_holder").css({
            ...
            'callback': function () {
                ...
                $('body').append($('#message'));
                $('.btn-send').css('display',
                    'inline-block');
                $('.btn-stop').css('display',
                    'inline-block');
                $('.btn-record').css('display',
                    'inline-block');
                $("#widget_holder1").append(
                    '<div style="position: absolute; z-index: 10000; width: 750px; margin-top: -400px; height: 400px; "></div>'
                );
                $("#widget_holder").offset($(
                    "#widget_holder1").offset());
    ...
    Library.prototype.show = function (key) {
        ...
        $(document).ready(function () {
            ...
            $('.faqlist>li.question').each(function () {
                var tis = $(this);
                var state = false;
                var answer = tis.next('li').hide().css(
                    'height', 'auto').slideUp();
                tis.click(function () {
                    state = !state;
                    answer.slideToggle(
    ...
    Library.prototype.loadExperiment = function (packaging, divSel) {
        ...
        function loadExp() {
            ...
            divSel = divSel || ".content_pane";
            var scripts = $.map(packaging['scripts'],
                includePath);
            var css = [];
            ...
            var callback = function () {
                ...
                packaging.browserStr = (new WhichBrowser())
                    .toString();
        ...
        var update_display = function () {
            ...
            for (i = 0; i < rows; i++) {
                ...
                for (j = 0; j < columns; j++) {
                    ...
                    (function (info) {
                        ...
                        exprBlock.click(function () {
                            if ($("#reg1").css(
                                ...
                                login_page +=
                                    '<div id = "force_login"><b>Please login or <a href="#" onclick="register();">register</a> to participate in this study.</b></div>';
                                login_page =
                                    login_page.replace(
                                        '"register"',
                                ...
                                login_page =
                                    login_page.replace(
                                        "If your family is new to Lookit, please",
                                        "");
                                login(login_page,
                                    info, this);
                            } else {
                    ...
                    }).createDelegate(this)(experiments[index +
                        (i * columns) + j]);
